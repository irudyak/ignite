...
public class KeyValuePersistenceSettings implements Serializable {
    ...
    public String getTableDDLStatement() {
        String keyColumnsDDL = keyPersistenceSettings.getTableColumnsDDL(null);
            ...
            if (keyColumn != null && !keyColumn.isEmpty()) {
                for (PojoField valField : valFields) {
                    if (keyColumn.equals(valField.getColumn()) &&
                            !CassandraHelper.isCassandraCompatible(keyClass, valField.getJavaClass())) {
                        throw new IllegalArgumentException("Value field '" + valField.getName() + "' shares the same " +
                                "Cassandra table column '" + keyColumn + "' with key, but their java classes are different. " +
                                "Fields sharing the same column should have the same java class as their type.");
                    }
                }
            }
                ...
                for (PojoField keyField : keyFields) {
                    for (PojoField valField : valFields) {
                        if (keyField.getColumn().equals(valField.getColumn()) &&
                                !CassandraHelper.isCassandraCompatible(keyField.getJavaClass(), valField.getJavaClass())) {
                            throw new IllegalArgumentException("Value field '" + valField.getName() + "' shares the same " +
                                    "Cassandra table column '" + keyColumn + "' with key field '" + keyField.getName() + "', " +
                                    "but their java classes are different. Fields sharing the same column should have the same " +
                                    "java class as their type.");
                        }
                    }
                }
            }
